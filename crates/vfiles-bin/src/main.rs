...
async fn run_serve(args: ServeArgs) -> anyhow::Result<()> {
        ...
        && !webdav_app.mount_prefix.is_empty()
    {
        ...
        app = app.nest_service(
            &mount,
            vfiles_webdav::apply_options_capabilities(
                vfiles_webdav::router_for_e2e(webdav_app).layer(vfiles_http::cors_layer(&config)),
            )
            .into_service(),
        );
    }
    ...
    async fn move_entry_with_condition(
        ...
        match condition {
            Some(condition) => self
                ...
                .map(|_| ()),
            None => {
                self.move_entry_with_overwrite(ns, from, to, uid, overwrite)
                    .await
            }
        }
    }
