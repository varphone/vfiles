...
async fn chunked_upload_history_and_download_round_trip() {
    ...
    assert_eq!(
        limited_history["data"]["nextCursor"],
        Value::String(version2.clone())
    );
    let cursor_history = app
        .request_as_admin(
            Request::builder()
                .uri(format!(
                    "/api/history?path=docs/note.txt&limit=1&cursor={version2}"
                ))
                .body(Body::empty())
                .expect("request should build"),
        )
        .await;
    assert_eq!(cursor_history.status(), StatusCode::OK);
    let cursor_history = response_json(cursor_history).await;
    assert_eq!(
        cursor_history["data"]["commits"].as_array().map(Vec::len),
        Some(1)
    );
    assert_eq!(
        cursor_history["data"]["commits"][0]["hash"],
        Value::String(version1.clone())
    );
    assert!(cursor_history["data"]["nextCursor"].is_null());
    assert_eq!(
...
async fn file_content_and_download_support_range_requests() {
    ...
    let head_with_range = app
        .request_as_admin(
            Request::builder()
                ...
                .body(Body::empty())
                .expect("HEAD range request should build"),
        )
        .await;
    assert_eq!(head_with_range.status(), StatusCode::OK);
    assert_eq!(
        head_with_range
            .headers()
            .get(header::CONTENT_LENGTH)
    ...
    assert!(response_bytes(head_with_range).await.is_empty());
    ...
    let current_date_if_range = app
        .request_as_admin(
            Request::builder()
                .uri("/api/files/content?path=docs/range.txt")
                .header(header::RANGE, "bytes=0-1")
                .header(header::IF_RANGE, &last_modified)
                .body(Body::empty())
    ...
    let stale_date_if_range = app
        .request_as_admin(
            Request::builder()
                ...
                .body(Body::empty())
                .expect("stale date If-Range request should build"),
        )
        .await;
    assert_eq!(stale_date_if_range.status(), StatusCode::OK);
    assert_eq!(
        response_bytes(stale_date_if_range).await.as_ref(),
        b"0123456789"
    );
...
#[tokio::test]
async fn multipart_upload_bounds_aggregate_unknown_metadata_and_cleans_temp_file() {
    let app = TestApp::new().await;
    let boundary = "----vfiles-aggregate-metadata-boundary";
    let mut body = format!(
        "--{boundary}\r\nContent-Disposition: form-data; name=\"file\"; filename=\"bounded-total.txt\"\r\n\r\nfile bytes\r\n"
    );
    let field_payload = "x".repeat(60 * 1024);
    for index in 0..5 {
        body.push_str(&format!(
            "--{boundary}\r\nContent-Disposition: form-data; name=\"extension-{index}\"\r\n\r\n{field_payload}\r\n"
        ));
    }
    ...
    let error = response_json(response).await;
    assert_eq!(error["code"], "VALIDATION_FAILED");
    assert_eq!(error["details"]["field"], "metadata");
    let temp_files = std::fs::read_dir(app._temp_dir.path().join("tmp"))
        .expect("upload temp directory should be readable")
        .count();
    assert_eq!(temp_files, 0, "rejected upload must remove its temp file");
...
#[tokio::test]
async fn multipart_upload_bounds_field_count_and_cleans_temp_file() {
    let app = TestApp::new().await;
    let boundary = "----vfiles-field-count-boundary";
    let mut body = format!(
        "--{boundary}\r\nContent-Disposition: form-data; name=\"file\"; filename=\"bounded-count.txt\"\r\n\r\nfile bytes\r\n"
    );
    for index in 0..16 {
        body.push_str(&format!(
            "--{boundary}\r\nContent-Disposition: form-data; name=\"extension-{index}\"\r\n\r\n\r\n"
    ...
    let error = response_json(response).await;
    assert_eq!(error["code"], "VALIDATION_FAILED");
    assert_eq!(error["details"]["field"], "multipart");
    let temp_files = std::fs::read_dir(app._temp_dir.path().join("tmp"))
        .expect("upload temp directory should be readable")
        .count();
    assert_eq!(temp_files, 0, "rejected upload must remove its temp file");
...
async fn cancelled_multipart_upload_removes_its_temporary_file() {
    ...
    let request = Request::builder()
        ...
        .header(
            header::CONTENT_TYPE,
            "multipart/form-data; boundary=----vfiles-single-upload-boundary",
        )
        .body(Body::from_stream(body_stream))
        .expect("request should build");
    let request_task = tokio::spawn(app.app.clone().oneshot(request));
    ...
    body_polled_rx
        .await
        .expect("multipart request body should continue streaming");
    request_task.abort();
    assert!(matches!(request_task.await, Err(error) if error.is_cancelled()));
        ...
        .expect("upload temp directory should be readable")
    {
        ...
        assert!(
            !filename.starts_with("single-upload-"),
            "cancelled multipart upload left temporary file {filename} behind"
        );
    }
}
