...
| MKCOL | ✅ r110' | 201/409 ✓ | - |
| MOVE | ✅ **r11/r12 覆盖语义；原子替换补强** | 201 新建 / **412**（F+存在）/ **204**（T + 缺省=T）✓；集合源的 `Depth` 缺省或 `infinity`，拒绝其他/重复值；Overwrite T 目标子树删除与源路径更新在 EntryRepo 单 SQLite 事务中提交；源、被覆盖目标的子树锁均须按资源标签提交 token | 客户端矩阵继续扩充 |
...
| 项 | 状态 | 备注 | 优先 |
| --- | --- | --- | --- |
| Depth 头 | ✅ | PROPFIND 支持 `0`/`1`，缺省 `infinity` 按有限深度策略拒绝（含 DAV 前置条件）；LOCK 支持 `0`/`infinity` 且缺省 `infinity`；不适用值 = 400 | - |
