...
| PROPFIND Depth | ✅ | 支持 `0`/`1`/`infinity`；省略按 RFC 默认 `infinity`；infinity 以批量子树查询返回递归 multistatus；非法值或重复字段返回 `400` | - |
...
| 项 | 状态 | 备注 | 优先 |
| --- | --- | --- | --- |
| Depth 头 | ✅ | PROPFIND 支持 `0`/`1`/`infinity` 且缺省 `infinity`；LOCK 支持 `0`/`infinity` 且缺省 `infinity`；不适用值 = 400 | - |
| If 头（锁条件） | ✅ **r7 + tagged 条件实证** | 支持未标记/URI-tagged、ETag、大小写不敏感的 `Not`、列表 AND/OR；任意合法 URI state-token 均参与求值，未知 token 只令该条件不匹配；**423/412 分码**；无效语法或重复字段返回 400，条件不匹配返回 412 | - |
...
**P1 进度**：
- ✅ r9 **认证热验缓存**（SHA-256 cred key 原文零存 ✗ 只缓存成功（失败不缓存）✗ 每次命中
  先查当前账号状态与密码哈希，因此停用、改密、改名和删除立即生效 ✓ Clone = Arc 共享
  ✓ Argon2 仅对缓存未命中或凭据变更的请求运行）
