...
| 62 | **cargo 命令禁止跨工具调用并发**（同消息多条各起 cargo = target 锁互等/排队 ✗✗ 叠加**自伤清场**（排队中杀 cargo = SIGKILL 把自己测试杀半途 = exit0 是**外壳完成的假绿** ✗✗✗ 正式 =**单 shell 内 `&&` 串行是一条、跨消息绝不并行 cargo**；清场只在 census 三空时做）| r3 |
| 63 | **长挂先归因再行动，验证一律带硬超时**（rsync test 挂 20min 我归因"锁等"连杀 N 轮 ✗✗ 真凶 =**测试死锁**（一条 `ps etime` + 进程名即穿 =未验证的第一归因错 ✗✗✗）→ 正式 =测试面首跑 `timeout 60 cargo test -p <crate>`（124 = 死锁秒判 ✗ 0 = 绿；避免分钟级悲剧）+ **`pgrep -f` 模式匹配自身 argv = 杀自己**（#56 二犯换形 ✗ 括号式 `[c]argo` 或按名 `-x`）| r3 |
| 64 | **BufReader 逐行新建 = 吞行后字节**（每行新建临时 BufReader 读 → 读到 `\n` 即弃、**后续字节留在已丢弃的内部缓冲里** ✗✗ 双连写一读 = 请求行 `\n` 被吞 → 双方互等 = 20 分钟死锁真身 ✗✗✗ 根治 =**单实例 BufReader 包 stream、写走 `get_mut()`、跨类型读也走 reader**（一发 0.00s 绿 ✓）| r3 |
| 65 | **测试 >5 分钟 = 必异常，立即停查不许等**（用户令 ✗✗ 实测正常量级 =**完整 26 套 9–10 秒**、单 crate 0–2s、各套 finished in 最慢 3.6s ✗✗✗ 任何"几分钟还没完"都是死锁/锁等/环境异常 =**当场 `timeout 300 cargo test` 封顶 + `ps etime` 归因**，禁止以"测试可能慢"自我安抚等下去（r3 那次 20 分钟死锁 = 本条的反面教材 ✗ 误判锁等连杀 N 轮）| r3（用户令） |
| 66 | **strace 子进程必须 `-f`、外层别套 timeout**（`strace timeout N cmd` = timeout fork 的 cmd **不在 trace 里** ✗✗ 只抓到 ld.so 读 ELF 的空转 = 一次全空 ✗✗ 正式 =**`strace -f -s 999999 -e trace=read,write,… -o out <直接二进制>`**（本地快不用 timeout ✗ `-f` 兜底 fork）= r5 直挂一把抓到双端全字节 wire 转录 ✓ | r5 |
