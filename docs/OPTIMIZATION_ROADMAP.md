...
- `client` 单测：30 个文件 / **194** 个用例通过；`vue-tsc`、`eslint`、`prettier`
  通过。
...
| 稳定性 | `FileBrowser.vue` 单文件 2788 行（已拆到 2277），职责过载  | 维护与回归风险             | `[~]` 见 §3.3 |
...
- `[ ]` 继续抽出桌面/移动工具栏模板（搜索框 + 高级筛选下拉约 400 行模板/样式），
  以及分享/历史/上传器的编排，目标 < 1500 行。
