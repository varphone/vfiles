...
- `client` 单测：27 个文件 / **170** 个用例通过；`vue-tsc`、`eslint`、`prettier`
  通过。
...
### 2.41 界面重设计（第一轮：整体视觉语言与文件列表，round 35，交互）
...
- `[x]` 第一轮：设计令牌 + 应用外壳 + 工具栏 + 文件列表/网格 + 空状态 + 品牌主色（round 35，见 §2.41）。
- `[ ]` 左侧导航栏（全部文件 / 最近 / 收藏 / 存储用量）——需要后端补充聚合接口后再做，
  避免放不可用的假入口。
- `[ ]` 右侧「详细信息」面板：选中文件时展示大小、类型、修改时间、所属目录与快捷操作。
- `[ ]` 顶栏重设计：把当前「快照切换器」收进下拉/胶囊按钮，主搜索框上移到应用栏
  （对齐 Drive/OneDrive 的全局搜索）。
- `[ ]` 移动端工具栏合并：内容区顶部搜索与底部操作栏存在重复入口，需统一为
  「顶部搜索 + 底部主操作」。
- `[ ]` 拖拽上传浮层、上传进度与通知的视觉统一。
