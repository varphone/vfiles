...
- `client` 单测：27 个文件 / **176** 个用例通过；`vue-tsc`、`eslint`、`prettier`
  通过。
...
| 稳定性 | `FileBrowser.vue` 单文件 2788 行（已拆到 2516），职责过载  | 维护与回归风险             | `[~]` 见 §3.3 |
...
- 验证：176 项前端测试、`vue-tsc`、`eslint`、生产构建通过；Playwright 回归确认
  详情面板仍显示当前条目（`docs`，7 个操作）、批量条在 0 选中时禁用「下载」、
  选中 1 项后变为「已选 1 项」且按钮可用、详情面板跟随选中变化、代码预览正常，
  全程无控制台报错。
...
- `[ ]` 继续抽出桌面/移动工具栏模板（搜索框 + 高级筛选下拉约 200 行模板/样式）、
  预览弹窗，以及分享/历史/上传器的编排，目标 < 1500 行。
...
- `[ ]` 左侧导航栏（全部文件 / 最近 / 收藏 / 存储用量）——需要后端补充聚合接口后再做，
  避免放不可用的假入口。
