...
### HTTP：要求 `If-Range` 日期具备强验证器精度

- 日期型 `If-Range` 现在须精确匹配 `Last-Modified`，且该日期至少早于当前时间 60 秒；近期日期可能无法区分同秒内的不同表示，不能据此返回部分内容。
- 回归覆盖旧日期匹配、近期日期回退完整响应，以及 HTTP API 端到端条件下载。

### HTTP：搜索只读取文件当前版本

- 文件名搜索原先为每条历史版本返回一条结果；内容搜索也会读取并匹配已被覆盖的旧版本，造成重复结果、无效 blob 读取和过期内容命中。
- 两类查询现在都只关联每个文件最大版本号对应的当前版本。
- 回归：覆盖同一路径文件后，文件名搜索只返回一条，旧内容不命中，新内容命中；完整工作区测试通过。
...
- 现在在第二个文件字段处直接返回验证错误，避免尺寸与落盘内容不一致，也避免意外覆盖目标。
- 回归：上传两个同名文件字段时返回 400，已有目标内容保持不变；`cargo test -p vfiles-http`。
...
- 现在按质量值依次尝试可用的 Brotli/gzip 变体，只有 identity 可接受时才回退明文；没有任何可用表示时返回 406。重复字段按列表合并解析。
- 回归：HTTP 单测和集成测试覆盖 `q=0`、高优先级变体缺失后的回退、406、重复字段及现有压缩路径。

### HTTP：静态资源按 Accept-Encoding 质量值协商

- 预压缩静态资源原先固定优先 Brotli，即使客户端给 gzip 更高的 `q` 值，也会忽略客户端偏好。
- 现在选择质量值最高的可用压缩编码，质量相同时继续优先 Brotli；明确拒绝 (`q=0`) 和非法质量值仍不会被选中。
- 回归：`cargo test -p vfiles-http`（28 个单测、88 个 API 集成测试通过）。

### HTTP：流式上传合并小块磁盘写入

- 原始 body 与 multipart 上传保持流式落临时文件，并在写入端增加 64 KiB 有界缓冲，减少高并发/小网络块场景下的磁盘写调用。
- 入库前显式 flush 并关闭文件，保证后续读取完整内容；超限或读取失败时仍清理临时文件。
- 回归：原始 body 上传、multipart 上传及完整 HTTP 测试集。

### HTTP：根目录分页去除临时排序

- SQLite 根目录分页原先按命名空间取行后，对“目录优先、名称升序”结果执行临时 B-tree 排序。
- 新增 `(namespace_id, kind='directory' DESC, path)` 排序索引，分页直接按索引顺序读取。
- 回归：SQLite `EXPLAIN QUERY PLAN` 断言命中新索引且不使用临时排序；HTTP 分页测试继续覆盖排序和页边界。

### HTTP：目录分页偏移量溢出

- 目录分页把客户端 `usize` 偏移量传给 `u32` SQL 接口时使用了截断转换；例如 `4294967296` 会回绕为 `0` 并返回首页。
- 转换改为饱和到 `u32::MAX`，越界偏移量保持为空页，并保留正确总数。
- 回归：`cargo test -p vfiles-http`。

### HTTP：Accept-Encoding 明确拒绝规则

- 静态资源压缩协商现在优先采用客户端对具体编码的声明；例如 `br;q=0, *` 不会选择 Brotli。
- 非法质量值及超出 `0..=1` 的值视为不可接受，通配符仍可用于未单独声明的编码。
- 回归：`cargo test -p vfiles-http`（24 个单元测试、87 个 API 集成测试通过）。
...
### 2.100 目录 ZIP 下载内存有界化（HTTP 性能与可靠性）
...
### 4.244x S3 区域校验（可选严校，round 46/256 ✨ pick 单点 22 委托全覆盖）

- **实装**：`VFILES_S3_REGION` 空 = 不校（兼容优先默认）✗ 设了严校 `Authorization` scope region 段 →
  不符 = `AuthorizationHeaderMalformed` 400（AWS 同形报文）；`S3Router::pick` 泛型化 `&S3Request<T>` 单点
  校验，**22 处委托 + list_buckets 统一覆盖**（读/写/列举路径真机均拒）。
- **真机验收（真 SDK，4 项）**：匹配区域全通；错区域 get/list_buckets/put 全 `AuthorizationHeaderMalformed`。
- **回归**：自写探针 **24/24** · 真 SDK **48/48**（严校开着）✗ 历史轮次即"空不校"回归。
- **债**：删除标记（S3）· per-dir merge / 符号链接设备 / basis 流式 / uid-gid（rsync）。

### 4.243x rsync filter 规则 flags 形修复（round 45/256 ✗ 真机抓出的静默失保）

- **实修**：wire 规则 = `<+|-><flags><space><pattern>`（取证 `-r *.probe`）；旧解析把 flags 吞进
  pattern → `--filter='P …'` 类保护**永不匹配 = 静默删文件**；现取**第一个空格后**为 pattern（flags 剥离 ✗
  pattern 内空格保留）。
- **真机对照**：带 filter → `keep.probe` 存活、未匹配文件正常删；无 filter → `keep.probe` 亦删（保护仅由
  规则生效）；wire 原始行入日志（`FILTER-RULE`）。
- **门禁**：单测 +flags 形断言 × workspace 全绿 × clippy 0 × build。
- **债**：per-dir merge（`:` 形）· 符号链接/设备持久化 · basis 流式 · uid/gid 存储。

### 4.242x S3 `versionId` 定向 + 版本控制配置（round 44/256 ✨ 版本化语义完整）

- **实装**：`Get/Head ?versionId=`（`resolve_version` 归属校验 → 404 `NoSuchVersion` ✗ `raw_commit`
  透传 = 正文/mime/size 同版本）· 响应 `x-amz-version-id` 恒回 · `DeleteObject ?versionId=`（新
  `delete_version` ✗ 非最新删行 / **最新版 `InvalidRequest` 明示需删除标记**）· `GetBucketVersioning`
  → Enabled（事实）· `PutBucketVersioning` 接受 Enabled、拒 Suspended · router 手写委托。
- **真机验收（真 SDK，12 项）**：定向读旧内容/旧 ETag、垃圾 id 404、定向删剩 2 版、已删 404、
  最新删拒且对象仍在、get=Enabled、put Enabled ok / Suspended 拒。
- **回归**：自写探针 **24/24** · 真 SDK **48/48** · clippy 0。
- **过程坑**：一次性补丁中 s3 文件**漏了 write**（部分改动只在内存）→ 真机 version_id 恒 None 才暴露；
  重放补丁（显式 write）后 12/12 全过。教训：多文件补丁逐文件 write + grep 断言落盘。
- **债**：删除标记 · region 校验（有意不做）。

### 4.241x S3 `ListObjectVersions` = 版本历史可见（round 43/256 ✨ 最新 ETag ≡ Head）

- **实装**：key 升序 + **key 内新版本在前**（`version_no desc`）· `VersionId`=版本 id hex（**最新版
  ETag ≡ `HeadObject` ETag**，同源自 version id）· `IsLatest`/`Size`/`LastModified` 逐版本 ·
  `key_marker`+`version_id_marker`+`max_keys` 续页（截断游标 = 最后已输出项）· delimiter 折叠跨请求不
  重复 · `encoding-type=url` 全字段编码 · `S3Router` 手写委托（同 r34 坑）。
- **真机验收（真 SDK，8 项）**：三覆盖 → 3 版本全列 + 首条 `IsLatest` 仅一条 · **最新 ETag == Head
  ETag** · size 正确 · 无跨键泄漏 · `MaxKeys=1` 双 marker **走全 3 条无重复** · delimiter 直出 ·
  `Get` 最新正文正确。
- **回归**：自写探针 **24/24** · 真 SDK **47/47**（+版本列表 2 项）。
- **意义**：我们**本就存有版本历史**却不可见 ✗ 备份/审计客户端现在能看到全部历史。
- **债**：`versionId` 定向读/删（现落最新版）· 删除标记 · `PutBucketVersioning` · region 校验（有意不做）。
...
- **实装**：`CreateBucket`（`default` → 409 `BucketAlreadyOwnedByYou`；其他桶名 → 400
  ...
  `S3Router` 补**手写委托**（生成器运行早于本轮 ✗ 新方法默认落 NotImplemented = 真机抓到的坑）。
- **真机验收（真 SDK，9 项）**：两台配置（默认 ns 非空 / tenant2 空视图）覆盖六种情形全对；
  拒后 `list_buckets` 桶仍在。
- **回归**：自写探针 **24/24** · 真 SDK **45/45**（+桶生命周期）。
- **意义**：工具的**建/删桶探测**有确定答案（此前 NotImplemented）✗ 409 语义与 AWS 逐字对齐。
- **债**：`ListObjectVersions`/`PutBucketVersioning`（真版本控制）· region 校验（有意不做）。

### 4.239x S3 列表 fetch-owner（round 41/256 ✨ V2 按需 / V1 恒带）

- **实装**：`ListObjectsV2` 的 `fetch-owner=true` → 逐对象带 `Owner{ID}`（本部署内条目均属该命名空间属主
  ✗ 不伪造 per-object 用户）；**V1 `ListObjects` 恒带 `Owner`**（AWS 同形）；与 `encoding-type` 可组合。
- **真机验收（真 SDK，4 项）**：V2 默认不带 / V2 `fetch-owner` 带非空 ID / V1 恒带 / 与编码组合。
- **回归**：自写探针 **24/24** · 真 SDK **44/44**（+fetch-owner）。
- **意义**：依赖 Owner 的工具（审计/权限脚本）不再拿到空对象。
- **债**：`ListObjectVersions`/`PutBucketVersioning`（真版本控制）· region 校验（有意不做）。

### 4.238x S3 `encoding-type=url` 列表编码协商（round 40/256 ✨ `%` 键可逆）

- **实装**：`ListObjects`/`ListObjectsV2` 支持 `encoding-type=url` → `Key`/`CommonPrefixes`/`NextMarker`/
  `NextContinuationToken`/回显 `Prefix`·`Delimiter` 一律百分号编码（未保留字符 + `/` 直出，其余按字节
  `%XX`），`EncodingType` 原样回显。
- **真机验收（真 SDK，5 项）**：默认原样 ✓；`url` 编码**可逆**（`unquote` 还原 `%`/空格/`+`/中文 4 键）✓；
  **关键判别**：键内 `%` → `%25`（否则客户端反解出错）✓；delimiter 折叠在编码下正确 ✓；含 `%` 键可直取 ✓。
- **回归**：自写探针 **24/24** · 真 SDK **43/43**（+编码协商）。
- **意义**：**特殊字符键**（空格/中文/`%`/`+`）在按规范实现的客户端上不再被误解 ✗ XML 安全性达成。
- **债**：`ListObjectVersions`/`PutBucketVersioning`（真版本控制）· region 校验（有意不做）。

### 4.237x S3 批量删逐键时间/大小条件（round 39/256 ✨ 条件面收口）

- **实装**：`DeleteObjects` 逐键条件扩到**三式**（`ETag` + `LastModifiedTime` + `Size`），任一不符 →
  该键 `PreconditionFailed`（不拖累整批）；`size` 经**父目录 `children_with_meta` 缓存**取版本大小。
- **真机抓到的坑**：库内 `created_at` 高精度 vs 客户端往返只到**秒** → 直接比较必不等；改经 **HTTP-date
  归一到秒**（`same_second`）后通过。
- **真机验收（真 SDK，6 项）**：全对删 / 只大小对删 / `size` 不符 412 / 时间不符 412 / 被拒者仍在 /
  只带正确时间重试成功。
- **回归**：自写探针 **24/24** · 真 SDK **42/42**（+时间/大小条件）。
- **意义**：批量清理的**条件面收口**（列表客户端可带 ETag/时间/大小任一作为并发护栏）。
- **债**：`ListObjectVersions`/`PutBucketVersioning`（真版本控制）· region 校验（有意不做）。

### 4.236x S3 条件读 = HTTP 304 缓存语义（round 38/256 ✨ 命中即不读 blob）

- **实装**：`check_get_conditions`（RFC 9110 §13 顺序：If-Match → If-Unmodified-Since → If-None-Match →
  If-Modified-Since）接入 `GetObject`/`HeadObject`：命中"未修改" → **`NotModified` 304**（**无正文 =
  不读 blob，缓存路径省 IO**）；`If-Match` 不符 / `If-Unmodified-Since` 已更新 → 412。
- **真机验收（真 SDK，10 项）**：`If-None-Match` 304/200 · `If-Match` 200/412 · `If-Modified-Since`
  304/200 · `If-Unmodified-Since` 412/200 · `HEAD` 亦 304 · `Range` 回归。
- **回归**：自写探针 **24/24** · 真 SDK **41/41**（+条件读）。
- **意义**：**浏览器/CDN/代理的 ETag 缓存协商**在本端生效（此前条件头一律被忽略 = 恒 200 全量重传）。
- **债**：`ListObjectVersions`/`PutBucketVersioning` · 逐键 `LastModifiedTime`/`Size` · region 校验。

### 4.235x S3 批量删逐键 ETag 条件（round 37/256 ✨ 并发删安全）

- **实装**：`DeleteObjects` 逐键读 `ObjectIdentifier.e_tag` → 与当前 ETag 对账，不符**只拒该键**
  （`Errors` 一项 `PreconditionFailed`，不拖累整批）；**缺失键 + 带条件 → 拒**（幂等仅对无条件删适用）；
  无条件键行为不变。
- **真机验收（真 SDK，7 项）**：条件匹配删 + 无条件删 / 不符者 412 / 被拒对象仍在 / 重试成功 /
  缺失键带条件 412 / 缺失键无条件幂等 `Deleted`。
- **回归**：自写探针 **24/24** · 真 SDK **40/40**（+逐键条件）。
- **意义**：批量清理路径也可做**乐观并发控制**（r28 只管单对象删，批量为常用路径）。
- **债**：`ListObjectVersions`/`PutBucketVersioning` · `LastModifiedTime`/`Size` 逐键条件 · region 校验。

### 4.234x S3 条件写 / 条件删（round 36/256 ✨ 乐观并发写）

- **实装**：`check_dest_conditions`（目标条目 `If-Match` / `If-None-Match` ✗ 缺失 = 不存在）→ 412
  `PreconditionFailed`；接入 `PutObject`（条件建/改）· `DeleteObject`（条件删）· `CopyObject` 的**目标**
  条件（与 `copy-source-if-*` 源条件独立）；`If-Match:*` = 必须存在，`If-None-Match:*` = 必须不存在。
- **真机验收（真 SDK，12 项）**：`If-None-Match:*` 首写 ok / 已存在 412 / **412 后内容未被改写**；
  `If-Match` 正确/过期/`*` → ok/412/ok；`DELETE If-Match` 过期 412（对象仍在）/ 正确 ok；拷贝目标条件
  新键 ok / 已存在 412。
- **回归**：自写探针 **24/24** · 真 SDK **39/39**（+条件写/删）。
- **意义**：**lost-update 防护**可用（此前无目标条件 = 并发写互相覆盖）。
- **债**：`ListObjectVersions`/`PutBucketVersioning` · `DeleteObjects` 逐键条件头 · region 校验（有意不做）。

### 4.233x rsync `--size-only` + `--modify-window`（round 35/256 ✨ 快跳判定与官方逐条对齐）

- **实装**：快跳判定补齐官方 `unchanged_file` 全语义 —— 默认 `size 等 && |Δmtime| <= modify_window`；
  `--size-only` 只比 size；`--modify-window=N` mtime 容差（非法值退 0）；`-I` 全关；`-c` 走校验和。
- **真机验收（每例带对照 · `--stats`）**：对照（默认，mtime 跨秒）**1** → `--size-only` **0** →
  `--modify-window=2` **0** → `--modify-window=0` **1**；`--size-only` + 同尺寸内容变更 0（**官方同义盲区**，
  `-c` 则 1）；拉回 `diff -r` 内容全同。
- **门禁**：`cargo test -p vfiles-rsync` 23/23 × workspace 全绿 × clippy 归零 × fmt × build。
- **意义**：`-a` 默认路径之外的**常见快跳调参**（大小优先/时钟容差）与官方一致 ✗ 时钟漂移/粗粒度
  mtime 的文件系统不再反复重传。
- **债**：符号链接/设备落地（domain 无条目类型）· `.rsync-filter` per-dir · basis 流式 · `--numeric-ids`。

### 4.232x S3 凭证→命名空间绑定 = 多租户隔离（round 34/256 ✨ 双向不可见实证）

- **实装**：`VFILES_S3_CREDENTIALS` 条目形扩为 **`access:secret[:命名空间slug][:ro]`**；新
  `NamespaceRepo::find_by_slug`（默认 `None` 桩零破 ✗ SQLite 一条 SQL，取属主作 S3 owner）；新增
  **`S3Router`**（`default_service` + `by_key: access → VfilesS3`）✗ `impl S3 for S3Router` **逐方法委托**
  19 个已实现操作（**现有服务逻辑零改动** = 由构造保证正确性）；slug 不存在 → 回落默认 + warn。
- **真机验收（真基建）**：DB seed 第二命名空间 `tenant2` → 默认凭证与 tenant2 凭证**互相只见自己的
  对象**（双向 `NoSuchKey`）；`tenant2:ro` 键可读不可写（`AccessDenied`）；拒后视图不变。
- **回归**：自写探针 **24/24** · 真 SDK **38/38**（无绑定）/ **40/40**（带绑定 ✗ 隔离 + 只读组合）。
- **意义**：S3 面可**按凭证切分命名空间** ✗ 多租户部署（此前所有凭证共享同一命名空间视图）。
- **债**：`ListObjectVersions`/`PutBucketVersioning` · router 委托样板 · region 校验（有意不做）。

### 4.231x S3 桶级探测三式（round 33/256 ✨ 客户端连接检查路径）

- **实装**：`HeadBucket`（存在 200 / 否则 `NoSuchBucket` 404）· `GetBucketLocation`（
  `LocationConstraint = us-east-1` 常量 `S3_REGION`，**签名区域不校验** = 客户端可用任意 region 配置）·
  `GetBucketVersioning`（无 `Status` = unversioned，与 AWS 未启用同形）。
- **真机验收（真 SDK）**：head 200 / 不存在 404 / location `us-east-1` / versioning 无 Status /
  `list_buckets`+put/get 回归全绿。
- **回归**：自写探针 **24/24** · 真 SDK **38/38**（+桶级探测）。
- **意义**：rclone / aws-cli 的连接检查与建桶流程不再撞未实现操作。
- **债**：凭证→命名空间绑定 · `ListObjectVersions`/`PutBucketVersioning`（真版本控制）· region 校验（有意不做）。

### 4.230x S3 条件复制 `x-amz-copy-source-if-*`（round 32/256 ✨ 并发拷贝安全）

- **实装**：`check_copy_conditions` 门控 4 头（RFC 9110 §13 + S3 语义）：`if-match` / `if-none-match`
  （ETag ✗ `*` 通配）+ `if-modified-since` / `if-unmodified-since`（时间上下界），不满足 → `PreconditionFailed`
  412；**`CopyObject` 与 `UploadPartCopy` 同门控**；`source_identity` 顺带把「源不存在」改判 `NoSuchKey`。
- **真机验收（真 SDK，11 项）**：if-match 正确/错误 → ok/412；if-none-match 命中/他值 → 412/ok；
  modified-since 过去/未来 → ok/412；unmodified-since 未来/过去 → ok/412；正文仍正确；
  `upload_part_copy` 条件同样生效。
- **回归**：自写探针 **24/24** · 真 SDK **37/37**（+条件复制）· `cargo test -p vfiles-s3` **9/9**（+条件单测）。
- **意义**：并发拷贝/备份的**乐观并发控制**可用（此前无条件头 = 永远照拷）。
- **债**：凭证→命名空间绑定（需 domain 按名查命名空间 + 逐请求解析）· region 校验。

### 4.229x S3 multipart 用户元数据（round 31/256 ✨ 元数据三路齐全）

- **实装**：`UploadStore::set/get_upload_custom_metadata`（存会话 `metadata.json` 的 `custom_metadata`）+
  `CreateMultipartUpload` 把 `x-amz-meta-*` 存**会话**（条目尚不存在）→ `CompleteMultipartUpload`
  落到 entry 属性（覆盖写语义）；**必须"先读会话再完成"**（完成会清会话目录 ✗ 顺序错即元数据丢失 =
  真机 23.1 抓到并修正）；Abort 会话销毁 → 元数据随之消失。
- **真机验收（真 SDK）**：Create 带元数据 → 分片 → 完成 → GET/HEAD **原样返回** + 正文完整；
  完成时无元数据 → **清空**（S3 语义：由 Create 决定）；Abort 不产生对象；之后普通 PUT 亦清空。
- **回归**：自写探针 **24/24** · 真 SDK **36/36**（+multipart 元数据）。
- **意义**：用户元数据 **PUT / CopyObject / multipart 三路齐全**（大文件上传也带得住元数据）。
- **债**：凭证→命名空间绑定 · region 校验 · `copy_source_if_*`。

### 4.228x S3 用户元数据 `x-amz-meta-*` 往返（round 30/256 ✨ 复用属性表零迁移）

- **实装**：复用 0006 `entry_properties`（前缀 `s3-meta:` ✗ **零迁移**）——`PutObject` 落元数据（覆盖写
  先清旧 = 「无元数据即清空」S3 语义）；`GetObject`/`HeadObject` 回填响应头；`CopyObject` 默认 **COPY 源
  元数据**、`MetadataDirective=REPLACE` 用请求值（无则清空）✗ 源条目不受影响；删 entry 随 FK 级联清。
- **真机验收（真 SDK）**：3 项元数据 PUT → GET/HEAD 原样返回；无元数据覆盖写 → **清空**；CopyObject
  默认 → **继承源**；`REPLACE` 带/不带元数据 → 替换/**清空**；源元数据未动。
- **回归**：自写探针 **24/24** · 真 SDK **35/35**（+4 元数据检查）。
- **意义**：客户端自定义元数据（`--metadata` / `x-amz-meta-*`）可**存取往返** ✗ 此前一律丢弃。
- **债**：multipart 的 `x-amz-meta-*`（会话无此字段）· 凭证→命名空间绑定 · region 校验 · `copy_source_if_*`。

### 4.227x S3 只读凭证 = 按凭证的读写权限（round 29/256 ✨ 9 个变更操作全门控）

- **实装**：`VFILES_S3_CREDENTIALS` 形扩为 `access:secret[:ro]`（`rw`/缺省 = 读写 ✗ 未知模式按读写 +
  warn）；s3s 把 SigV4 的 `Credentials{access_key}` 放进 `S3Request.credentials` → `require_write()`
  命中只读键即 `AccessDenied`，覆盖 **9 个变更类操作**（Put/Delete/DeleteObjects/CopyObject/
  CreateMPU/UploadPart/UploadPartCopy/CompleteMPU/AbortMPU）；读类不受限。
- **真机验收（3 键同服）**：`rw` 写读 ✓ · `ro` 读列 ✓ · `ro` 对 8 类变更操作 **全部 `AccessDenied`** ✓ ·
  拒后数据未变 ✓ · 单对 env 键仍 rw ✓。
- **回归**：自写探针 **24/24** · 真 SDK **31/31**（`boto_probe.py` 第 6 参 = 第二凭证只读标志，
  ro/rw 两种服务端配置各跑一遍均 31/31）。
- **意义**：S3 面可发**只读凭证**给消费者/备份客户端（此前只有"全权"一档）。
- **债**：凭证→命名空间绑定（需 domain 加按名查命名空间）· region 校验 · `copy_source_if_*`。

### 4.226x S3 列表 SQL 逐页 = 不物化整桶（round 28/256 ✨ 内存 O(max_keys)）

- **实装**：`EntryRepo::files_with_meta_page`（默认 = `files_with_meta` 过滤切片 ✗ 桩零破；**SQLite 覆写 =
  一条 SQL** `kind='file' AND path > ? ORDER BY path LIMIT ?`）+ **`ListCollector` 流式收集器**
  （prefix 过滤 → delimiter 连续同组去重 → `after` 独占续页 → 收 `max+1` 判截断；`prefix_upper_bound`
  越过即停扫）+ `list_page` 1000/批拉取收满即停 → **内存 = O(max_keys) 而非 O(桶)**。
- **等价性**：`build_entries`+`paginate` 降为 `#[cfg(test)]` **参考实现** ✗ 新单测对 6×2 组参数按
  `max=3` **逐页走全**，断言 key 序列与参考**完全一致且无重复**。
- **真机验收（500 对象 ×10 目录）**：`PageSize=100` = 5 页 500 key 无重无漏 + 升序；`delimiter=/` = 10
  前缀；V1 `MaxKeys=120` = `IsTruncated`+`NextMarker`+续页；`StartAfter` 严格大于；`MaxKeys=1` **500 次
  走全 500 key（1.9s）**。
- **回归**：自写探针 **24/24** · 真 SDK **30/30**（+V1 marker / StartAfter / 单键续页）。
- **意义**：十万级桶的列表请求**不再随桶大小吃内存**（此前每请求物化全桶）。
- **债**：访问键绑用户 · region 校验 · `copy_source_if_*` 条件头。

### 4.225x S3 `UploadPartCopy` = 大对象跨键拷贝（round 27/256 ✨ 真 SDK 27/27）

- **实装**：`UploadPartCopy`（`x-amz-copy-source` + `x-amz-copy-source-range`）：同桶源校验 → 读源字节
  → （可选）`bytes=start-end` 闭区间切片（亦支持开尾/后缀 ✗ 尾越界夹取、起点越界 → `InvalidRange`）
  → `upload_part` 落分片 → 返回 `CopyPartResult{ETag=MD5(分片), LastModified}`。
- **实证（真 SDK）**：1MiB 源分两段复制 → 完成 → `GetObject` **逐字节同**；`ListParts` ETag 与复制返回
  逐一相等；无 range 整对象复制同；起点越界被拒。
- **回归**：自写探针 **24/24** · 真 SDK **27/27**（+UploadPartCopy）。
- **意义**：拷贝面 **`CopyObject`（小对象）+ `UploadPartCopy`（大对象/分片）双式齐全** ✗
  rclone/aws-cli 的大文件服务端拷贝可用。
- **债**：SQL 级 prefix/limit 分页 · 访问键绑用户 · region 校验 · `copy_source_if_*` 条件头。

### 4.224x rsync `-a` 修复 + size+mtime 默认快跳（round 26/256 ✨ 最常用调用从不可用到 0 传输）

- **修掉的真 bug**：`rsync -a`（最常用）此前**整条不可用**（历轮只测过 `-r`）——flist 缺 uid/gid 字段、
  符号链接 target 位置错（官方序在 uid/gid/rdev **之后**）、`XMIT_HLINKED`/rdev/atime 未处理、
  **flist 后的 uid/gid 名列表**未收发。收发两侧均补齐（发送端须打 MSG_DATA 帧 = 真机 rc12 的坑）。
- **size+mtime 默认快跳**（官方 `unchanged_file` 同义）：迁移 `0007_entry_source_mtime.sql` +
  `set_version_source_mtime`（**不动 `create_version` 签名** = 10 处调用零改）；push 记源端 mtime；
  `EntryChildMeta.source_mtime` 带出；`RsyncBackend` 增 **`stat`**（连接级 `OnceCell` 缓存 = 一次
  `files_with_meta` 覆盖全树 ✗ 非逐文件点查）；`-I` 关闭、`-c` 走校验和。
- **下载侧 mtime 保真**：`collect_flat` 优先 `source_mtime` → 客户端 `-a` 拉到源端 mtime，二次下载自带快跳。
- **真机验收（`--stats`）**：`-a` 首推 3 → 无改动重推 **0** → `-a -I` 3 → `touch` 后 1 → 再 **0**；
  `-a` 拉回后再拉 **0**（双侧 mtime 相等）；同秒同尺寸改动 0（**与官方一致** ✗ `-c` 则 1）；跨秒改动 1→0。
- **门禁**：`cargo test -p vfiles-rsync` **23/23**（+uid/gid 与 id-list 真机帧回归）× workspace 全绿 ×
  clippy 归零 × fmt × build。
- **意义**：`rsync -a`（备份默认姿势）+ 默认快跳可用 ✗ 与增量/双向/认证/镜像合起来 = **可日常使用的备份端**。
- **债**：符号链接/设备落地（已解析 ✗ domain 无条目类型）· `.rsync-filter` · basis 流式 · `--numeric-ids` 映射。

### 4.223x rsync `-c` 校验和快跳 = 未变文件 0 传输（round 25/256 ✨ 真机 `--stats` 佐证）

- **实装**：`-c/--checksum` 时解析 flist **末字段**整文件 MD5（`flist_csum_len`=16 ✗ 仅普通文件）入
  `FlatEntry.file_sum`；收端（generator 角色）取本地 basis 后 `MD5(basis) == file_sum` → **不发请求**
  （等价官方 `generator.c` `memcmp` ✗ basis 已在 delta 路径读取 = 零额外 IO）；args 加 `--checksum`/`-c`/`--no-c`。
- **真机验收（`--stats`）**：首推（无 `-c`）3 文件 / 1,572,866 literal → 无改动 `-c` 重推 **0 文件 / 0 literal**
  → 改 1 文件后 `-c` **1 文件** / 1,024 literal（+1,047,552 matched）→ 再无改动 **0 文件**；拉回
  `diff -r` 内容全同。
- **回归**：`--delete` + `--exclude` 保护 / 无 exclude 删除 / 空目录结构 全保持。
- **门禁**：`cargo test -p vfiles-rsync` **22/22**（flist `-c` 往返新增校验和断言）× workspace 全绿 ×
  clippy 归零 × fmt × build。
- **债**：默认快跳（size+mtime）需存**源 mtime**（结构性改动）· 符号链接/设备 · `.rsync-filter` · basis 流式。

### 4.222x S3 `ListMultipartUploads` = multipart 六式齐全（round 24/256 ✨ 真 SDK 26/26）

- **实装**：`UploadStore::list_upload_sessions`（扫上传目录 ✗ 损坏目录跳过）+ app 命名空间过滤 →
  `ListMultipartUploads` 只列 `Receiving`；支持 `prefix` / `delimiter` 折叠 / `max-uploads` /
  `key-marker`+`upload-id-marker` 续页（同 key 多会话按 upload id 定序）。
- **实证（真 SDK）**：5 个进行中上传全列出（含 `Initiated`）→ `prefix` 过滤 4 条 → `delimiter=/`
  `lmu/sub/` 折叠 → `max-uploads=2` 截断 + marker 续页无重 → abort 后消失。
- **回归**：自写探针 **24/24** · 真 SDK **26/26**（+2）。
- **意义**：multipart **六式齐全**（Create/UploadPart/Complete+校验/Abort/ListParts+ETag/ListUploads）✗
  未完成上传可列举/清理（客户端中断残留可见）。
- **债**：SQL 级 prefix/limit · 访问键绑用户 · region 校验 · `UploadPartCopy`。

### 4.221x S3 multipart 完整性 = part ETag + 完成校验（round 23/256 ✨ 真 SDK 24/24）

- **实装**：`UploadStore::read_upload_part`（+ app 包装）✗ `ListParts` 逐分片算 `ETag = MD5(分片)`；
  `CompleteMultipartUpload` ① **ETag 对账**（客户端回显 vs 存储分片 MD5，不符 `InvalidPart`）
  ② **列出集合必须等于已存集合**（否则拼接按全部已存分片 = 内容静默错位）。
- **实证（真 SDK）**：4×256KiB 分片 `list_parts` ETag **逐一相等**；篡改 ETag → `InvalidPart`；
  只列子集 → `InvalidPart`；正确完成 → 1,048,576 B 逐字节同。
- **回归**：自写探针 **24/24** · 真 SDK **24/24**（+2 multipart 完整性）。
- **意义**：multipart 从"能传"升到**可校验**（此前 part ETag 恒缺、完成时不校验 = 传输损坏无法发现）。
- **债**：`ListMultipartUploads` · SQL 级 prefix/limit · 访问键绑用户 · region 校验 · `UploadPartCopy`。

### 4.220x rsync 空目录落地 = 目录结构完整（round 22/256 ✨ `find` 结构全同）

- **实装**：`RsyncBackend` 增 **`mkdir`**（bin = `workspace.create_directory` ✗ 已存在视为成功）→ push
  循环对 flist 的**目录条目显式建目录**（此前跳过 = 空目录丢失；非空目录靠文件提交隐式创建故未暴露）。
- **真机验收**：推 `empty/` + `nest/a/b/c/`（嵌套空）+ `full/x.txt` → `--list-only` 全在 → 拉回
  **`find` 结构 IDENTICAL** + 内容同；回归推/delta/`--delete` 均保持（1,024 literal / 1,047,556 matched）。
- **门禁**：`cargo test -p vfiles-rsync` 22/22 × workspace 全绿 × clippy 归零 × fmt × build。
- **意义**：rsync 备份的**目录结构保真**补齐（此前空目录静默丢失 = 备份不完整）。
- **债**：符号链接/设备（domain 无对应条目类型）· `.rsync-filter` per-dir 规则 · mtime/size 比较 ·
  basis 流式。

### 4.219x S3 列表一条 SQL 消 N+1 = 大桶可扩展（round 21/256 ✨ 250 对象 11.7ms）

- **实装**：新增 `EntryRepo::files_with_meta`（默认 = `find_all` 过滤 ✗ 桩零破；**SQLite 覆写 =
  一条 SQL**：`entries WHERE kind='file'` + 最新版本 size/content_type 标量子查询）✗ S3
  `collect_objects` 由**逐目录递归**（目录数条查询）改为**一次取全 + 内存排序**。
- **实证**：250 对象 ×10 目录全列 **11.7ms**；`max-keys=100` = 3 页 250 key **无重无漏**；
  `delimiter=/` = 10 个 CommonPrefixes；字典序正确。
- **回归**：自写探针 **24/24** · 真 SDK **22/22**（新增 120 对象分级分页 + 4 前缀 delimiter）。
- **含义**：列表成本从「目录数」降到「一次查询」✗ 十万级桶不再随目录数劣化。
- **债**：SQL 级 prefix/limit（现全量入内存）· `ListMultipartUploads` · 访问键绑用户 · region 校验 ·
  part 哈希持久化。

### 4.218x rsync filter/exclude 规则 = `--delete` 可信镜像（round 20/256 ✨ 保护有对照）

- **实装**：`--delete` 场景客户端先发的 **filter list** 不再丢弃 → `FilterRule` 解析
  （`- pat` 保护 / `+ pat` 解除 / 尾 `/` 仅目录 / 首 `/` 或含 `/` 锚定；其余类型跳过）+ `wildmatch`
  子集（`*` 不跨 `/`、`**` 跨、`?`、字符类）+ `is_excluded` 首条命中语义 + **目录子树保护传播** +
  `--delete-excluded` 忽略保护。
- **真机验收（每例对照）**：`--delete` 无 exclude = `removed=2` → `--delete --exclude='*.tmp'` =
  `keep.tmp` **被保护**（无删除）→ `--delete --delete-excluded --exclude='*.tmp'` = `removed=2`；
  `--delete --exclude='secret/'` = `secret/deep/s.txt` **整树保护** → 无 exclude 时整树删除。
- **门禁**：`cargo test -p vfiles-rsync` **22/22**（wildmatch 12 断言 + 解析/保护 9 断言）×
  workspace 全绿 × clippy 归零 × fmt × build。
- **意义**：`--delete` 从"会删不该删的"升到**可信镜像**（备份安全基线）。
- **债**：per-dir merge 文件 / `P`/`H`/`S`/`R` 规则类型 / `--filter` 完整语法 · mtime/size 比较 ·
  符号链接/设备/空目录 · basis 流式。

### 4.217x rsync `--delete` 镜像 + 后端 trait 化（round 19/256 ✨ 删本地后重推 `diff -r` 全同）

- **`--delete`**（`--delete`/`--delete-before/during/delay/after/excluded`/`--del`）：
  识别 args → **先读 filter list**（客户端 receiver_wants_list=true 才发 ✗ 规则本版忽略）→ 收 flist →
  传文件 → `backend.list(base, recursive)` 列目标 → 名字不在源集合者 = extra → 一次
  `backend.delete`；`--delete` 无 `-r` 记 warn 跳过（防过度删除）。
- **架构重构**：三闭包（list/read/write）→ **`RsyncBackend` trait**（`list`/`read`/`write`/`delete`
  四法 + `async_trait`）✗ `handle_conn<S, B: RsyncBackend>` 签名收敛（不再堆泛型）；bin 侧
  `RepoBackend`（domain 链）+ 测试侧 `FakeBackend`（内存表）。
- **真机验收**：推 3 项 → 删本地 1 项 → **无 `--delete`** 重推 `b.txt` 保留（对照）→ 同状态
  **加 `--delete`** 重推 `b.txt` 消失 + 拉回 **`diff -r` MIRROR IDENTICAL**；删整个 `sub/` 后
  `--delete` = 整树消失；兄弟前缀 `del2/` 与子路径 `del2/inner/` **隔离正确**；回归推/取 delta
  均 1,024 literal / 1,047,552 matched + 重建正确。
- **门禁**：`cargo test -p vfiles-rsync` **20/20**（`FakeBackend` 全绿）× workspace 全绿 × clippy
  归零 × fmt × build。
- **意义**：rsync **镜像（备份最核心用法）**可用 ✗ 与增量/双向/认证合起来 = 可日常使用的备份端。
- **债**：filter/exclude 规则未解析（`--exclude` + `--delete-excluded` 语义不完整）· 无 `-r` 时跳过
  删除 · 不比较 mtime/size · 符号链接/设备/空目录 · basis 全量入内存。

### 4.216x S3 多凭证 + CopyObject + Content-Type 贯通（round 18/256 ✨ 探针 24/24 · 真 SDK 21/21）

- **多凭证**：`VFILES_S3_CREDENTIALS=access:secret,...`（与单对并存）✗ `EnvAuth` 改 `access → secret`
  表；未知 key = `InvalidAccessKeyId` 403（多客户端/密钥轮换可用）。
- **`CopyObject`**：`PUT` + `x-amz-copy-source` → 读源字节 + 上传链落目标；`MetadataDirective=REPLACE`
  覆盖 ContentType，否则沿用源；返回 `CopyObjectResult{ETag, LastModified}`。
- **Content-Type 贯通（本轮抓到的真 bug）**：上传链此前 `let _ = mime_type` + 提交按扩展名猜 ✗
  实测 `application/x-thing` 回读成 `application/octet-stream`（CopyObject 探针抓到）→
  `UploadSession.mime_type` 全链贯通（domain 字段 + sqlite store 元数据 + 提交优先声明值）✗
  **同时惠及 WebDAV/HTTP 上传**。
- **门禁**：`sigv4_probe.py` **24/24**（+CopyObject）· `boto_probe.py` **21/21**（+CopyObject +
  第二凭证可用 + 未知凭证被拒，脚本支持可选第 2 凭证参数）× workspace 全绿 × clippy × fmt × build。
- **债**：访问键未绑定用户（授权粒度）· `ListMultipartUploads` · region 校验 · 大桶 SQL 分页 ·
  part 哈希持久化。

### 4.215x rsync 收端 delta = 双向增量（round 17/256 ✨ 推 3MiB 改 1KB 只传 1.7KB）

- **实装**：push 请求不再恒发 `sum_head` 全零 —— 以**本地现有内容为 basis**：
  `block_size`（sqrt 夹取 700..32KiB）+ `build_block_sums`（弱 sum1 int32 + 强 `MD5(seed‖块)`×16）
  随请求发出；收端把 token 流原样缓冲后交 **`apply_tokens`** 重建（`>0` literal / `<0` 匹配 basis 块
  `idx=-t-1` / `0` 终结）；无 basis 仍全 literal。
- **真机验收**：首推 3,145,728 literal / 0 matched → 改 1000B 重推 **1,773 literal / 3,143,955 matched
  （99.94%）** → 内容同重推（`-I`）**0 / 2,097,152（100%）** → 多文件递归 3 文件改 1 个
  **724 / 1,572,146** ✗ 推后拉回 `cmp`/`diff -r` **全同**。
- **门禁**：`cargo test -p vfiles-rsync` **20/20**（新增「basis → 块校验和 → 发送端 token →
  `apply_tokens` 重建 == 新内容」闭环单测 + 无 basis 分支）× workspace 全绿 × clippy 归零 × fmt × build。
- **意义**：rsync **双向均增量**（下载 r11 + 上传 r14）✗ 备份双向带宽收益齐备。
- **债**：不比较 mtime/size（等价 `-I`）· 符号链接/设备/空目录 · `--delete` · basis 全量入内存。

### 4.214x rsync secrets 认证 + 写门控 = 匿名写窗口关闭（round 16/256 ✨ 真机四态验证）

- **实装**：rsync daemon **challenge-response 认证**（`@RSYNCD: AUTHREQD` → `user base64(HASH(pass‖challenge))`）：
  - 摘要协商 `sha512 / sha256 / md5`（banner 同步 `@RSYNCD: 30.0 sha512 sha256 md5` ✗ 去掉未实现的
    sha1/md4）；`auth users` 支持 `:ro` / `:rw` / `:deny`
  - **无填充 base64**（`STANDARD_NO_PAD` ✗ rsync `base64_encode` 不带 `=`；带填充必失配 = 首测全败
    的真因，用真机转录帧算出 sha512 期望值定位）
  - 常量时间比较；随机 challenge = `base64(24B /dev/urandom)`
- **写门控**：`VFILES_RSYNC_WRITABLE`（默认 **false**）→ 只读模块收 push = 多路复用 `MSG_ERROR`
  `ERROR: module is read only`（官方 `do_server_recv` 同形）+ 关闭。
- **配置**：`VFILES_RSYNC_WRITABLE` / `VFILES_RSYNC_AUTH_USERS` / `VFILES_RSYNC_SECRETS_FILE`
  （`.env.example` 已记档）。
- **真机验收（四态）**：正确口令 list RC0 · 正确口令 push（alice `:rw`）RC0 → 拉回 **`diff -r`
  IDENTICAL** · 错误口令 `@ERROR: auth failed on module files` RC5 · bob `:ro` 可 list 不可 push
  RC12 · 匿名+只读 push 被拒 · 匿名+writable=true 显式 opt-in 回归 OK。
- **门禁**：`cargo test -p vfiles-rsync` **19/19**（新增 sha512 响应真机转录 golden + verify 三态）·
  workspace 全绿 · clippy 归零 · fmt · build。
- **意义**：r12 的「匿名可写」风险关闭 ✗ rsync 面达到可对外暴露的基线。

### 4.213x rsync push 打通 = 双向闭环（round 15/256 ✨ 推→列→拉 `diff -r` 全同）

- **实装**：`recv_file_list`（`recv_file_entry` 逆序解码 ✗ `lastname` 前缀压缩重建 + 差分态）·
  `data_varint`/`data_varlong`/`int_byte_extra`（io.c 移植）· `handle_conn` 第五参 `write_file`
  闭包（bin 接 `UploadService` 上传链）· `if args.is_sender { download } else { push }` 双径。
- **关键点**：push 的 **ndx = 排序后下标**（复用 `sort_flist`）；请求 `sum_head` 全零 → 客户端
  **全 literal**（收端不做 delta）；push **不读 filter list**（客户端 sender 且无 delete 时不发）。
- **真机验收**：`rsync -r /tmp/src/ …/files/push1/` RC0（日志 `rsync push 完成 files=3`）→
  `--list-only` tree 正确 → 拉回 + **`diff -r /tmp/src` IDENTICAL**；3MiB 随机 + 深嵌套 + UTF-8 名
  推送拉回 **3/3 `cmp` 逐字节同**；重复推送幂等 RC0；推后该文件下载 `-I` = Literal 1,768 /
  Matched 3,143,960（r11 delta 无回归）。
- **门禁**：`cargo test -p vfiles-rsync` **18/18**（新增 wire 读写互逆 + flist 编解码互逆）·
  workspace 全绿 · clippy 归零 · fmt · build。入档 `golden/push_wire_r12.md`。
- **意义**：rsync **四要素（列 / 取 / 增量 / 推）齐备** ✗ 备份双向可用。
- **债（明确且重要）**：**无认证 = 匿名可写**（`VFILES_RSYNC_ENABLED=true` 显式开启 = 运维自担；
  secrets 认证 = 后续轮最高优先）· 收端无 delta · 不比较 mtime/size · 符号链接/设备/空目录不落地 ·
  `--delete` 未支持。

### 4.212x S3 流式 PUT + 批量删 = 客户端日常动线补齐（round 14/256 ✨ 自写 23/23 · 真 SDK 19/19）

- **流式 PUT**：`PutObject` 在 Content-Length 已知时用 `tokio_util::io::StreamReader` 把请求体直接
  接进 `complete_upload_from_stream`（**不再全量入内存** ✗ 8MiB 实测往返字节同）；未知长度回退聚合；
  响应补 **ETag + Size**（此前 PUT 无 ETag）。
- **`DeleteObjects` 批量删**：逐键 `find_by_path` 分区（存在 / 缺失 / 非法）→ 存在者**一次
  `delete_entries`**（单快照）；缺失键按 S3 幂等语义记 `Deleted`；`Quiet` 抑制 `Deleted` 条目；
  批量失败逐键回退并逐键报 `Error`。
- **踩坑（探针抓到）**：`delete_entries` 遇**任一**路径缺失即整体 `NotFound` 且不删任何键 ✗ 旧批量
  写法"报成功但没删"（6.4 失败）→ 修正先分区再批量（6.4 转绿）。
- **实证**：`sigv4_probe.py` **23/23**（新增 `DeleteObjects` 含缺失键 + 删后列表清空）·
  `boto_probe.py`（真 AWS SDK）**19/19**（新增 **8MiB 流式 PUT 往返** + `DeleteObjects` 全报删/Quiet）。
- **意义**：`rclone sync`/`aws s3 rm --recursive`/大文件上传三条最常用动线齐备。
- **债**：`ListMultipartUploads` · per-user 凭证 · region 校验 · 大桶 SQL 分页 · part 哈希持久化。

### 4.211x rsync 真 delta 落地 = 增量重建逐字节正确（round 13/256 ✨ 弱 sum1 + 强 sum2 全实证）

- **实装**：接收端块校验和不再丢弃 → `BlockSum{sum1,sum2,len}` + `build_delta_tokens`（滚动弱
  校验和命中 → 强校验确认 → 匹配 token `-(idx+1)`；未命中段 literal；`int32(0)` 终结）+
  `checksum1_signed`（**signed char** 逐字节滚动）+ `md5_seeded`（块强校验和 = `MD5(seed_le‖块)`
  前 `s2length` 字节）。
- **两式辨析（真机实证入档 `golden/delta_checksum_r11.md`）**：块强校验和**含 seed**（`get_checksum2`
  `proper_seed_order=1`），整文件校验和**不含 seed**（streaming `sum_update`）✗ 二者不同、
  此前只知后者。弱校验和 `0x760cfeda` 与转录逐位同（`s1=Σ(i8)b`、`s2=Σs1`）。
- **真机验收（`--stats` 数字即证据）**：2,254,652B CSV —— 首轮全 literal 2,254,652；改 1000B 后
  `-I` = **Literal 2,992 / Matched 2,251,660（99.87%）**；内容同再跑 = **Literal 0 / Matched 100%**；
  递归树 4 文件改 500B = **Literal 1,448 / Matched 70,688,460** ✗ 落地内容 **SHA-256 全 = blobs 表**。
- **门禁**：`cargo test -p vfiles-rsync` **16/16**（真机校验和 golden + 全匹配 token 流 + 部分匹配
  混合）× workspace 全绿 × clippy 归零 × fmt × build。
- **意义**：rsync 三要素（列 / 取 / 增量）齐备 ✗ 备份场景的带宽收益真实可用。
- **r14 债**：收端 push · secrets 认证 · 大文件流式（现全量入内存）· `--checksum`/压缩。

### 4.210x S3 multipart 上传打通 = 大文件客户端路径可用（round 12/256 ✨ 真 SDK 14/14）

- **实装**：`CreateMultipartUpload` / `UploadPart` / `CompleteMultipartUpload` /
  `AbortMultipartUpload` / `ListParts` 五面 ✗ 复用既有 `upload_sessions` 分片存储（无新表）：
  - app 层 `commit_upload_stream` 增 `enforce_size: bool`（PUT 路径传 true）+ `init_multipart_upload`
    （未知总大小：`declared_size=0`/`total_chunks=0`）/ `complete_multipart_upload`（跳过量校验）/
    `list_upload_parts`
  - S3 层 part 号 1..=10000 ↔ 内部零基 `partNumber-1`；`UploadPart` 返回 **part MD5 ETag**；
    `Complete` 先校验列出 part 均已上传 → 拼接 → 落库（新版本链）
- **实证**：
  | 通道 | 结果 |
  | --- | --- |
  | `scripts/sigv4_probe.py`（自写 SigV4） | **21/21**（r2 九 + r3 八 + r4 四） |
  | `scripts/boto_probe.py`（**真 AWS SDK**） | **14/14**（4×1MB 分片 → complete → **拼接字节逐字同** → abort） |
- **意义**：rclone / aws-cli 对超过阈值的文件自动走 multipart ✗ 此前大文件上传必失败 = 本轮补齐；
  配合 r3 的列表面（delimiter/续页/Range）与 r2 的版本链，S3 面达到"真实客户端可日常使用"。
- **债**：part 零基连续约束（跳号失败）· 完成不校验 part ETag / ListParts 无 ETag（part 哈希未持久化）·
  `ListMultipartUploads` 未实现 · 最终 ETag = 版本 id（非 AWS 复合形）· put 流式直连 · per-user 凭证 ·
  region 校验 · 大桶 SQL 分页。

### 4.209x S3 列表面商业级完备 = 自写探针 17/17 + 真 AWS SDK 9/9（round 11/256 ✨）

- **实装**（`crates/vfiles-s3/src/lib.rs`）：
  - `ListObjectsV2` **全语义**：prefix · delimiter→**CommonPrefixes**（只折一层，AWS 语义）·
    **continuation-token** · start-after · max-keys（尊重请求、负数 InvalidArgument、上限 1000）·
    is-truncated · **next-continuation-token** · key-count
  - `ListObjects`（V1）：marker / delimiter / max-keys / next-marker / common-prefixes
  - `Object` 带 **size / last-modified / ETag**；`GetObject`/`HeadObject` 补 **last-modified** +
    **HTTP Range → 206 + Content-Range**（s3s 见 `content_range` 自置 206 ✗ `Range::check` 负责
    夹取与 416 InvalidRange）
  - 纯函数 6 单测：delimiter 折一层 / 分页续页无重无漏 / 满页不截断 / max-keys 规则 / range 四态
  - clippy 归零（output 构造统一 struct-literal ✗ 清 5 处 field_reassign 旧债）
- **双通道实证（入仓可复演）**：
  | 通道 | 结果 |
  | --- | --- |
  | `scripts/sigv4_probe.py`（自写 SigV4） | **17/17**（r2 九式 + 元数据/delimiter/分页 12a+12b/V1/Range 206/Range 416/HEAD Last-Modified） |
  | `scripts/boto_probe.py`（**真 AWS SDK** botocore：签名+XML+**paginator**+Range） | **9/9**（SDK `get_paginator` PageSize=1 = 4 页 4 key 无重） |
- **意义**：rclone / aws-cli / Cyberduck 等真实客户端依赖的「目录折叠 + 续页 + 元数据 + 断点
  Range」四面全通 ✗ 从"能读能写"升到"客户端可用"。
- **r12 债**：put 流式直连 · multipart upload · `DeleteObjects` 批量删 · per-user 凭证 · region
  校验 · 大桶 SQL 分页（现全量枚举后内存分页）。

### 4.208x rsync 整文件下载落地 = 能列 → 能取（round 10/256 ✨ 真机内容 SHA-256 全对）

- **下载探针兑现**：真 `rsync 3.2.7` 取我方 daemon 内容——
  | 探针 | 结果 |
  | --- | --- |
  | 单文件 `rsync …/files/说明.md /tmp/` | RC0 + 内容逐字同 ✓ |
  | `rsync -r …/aka/foo/bar/hallo/ /tmp/`（4 文件 / 70MB / 嵌套 2 层） | RC0 + **4/4 SHA-256 = blobs 表** ✓ |
  | 二跑 `-I`（接收端发块校验和 count>0） | RC0 + 4/4 哈希仍全对 ✓ |
- **关键校正**：checksum 清单**收敛 `"md5"`**（官方全清单会协商 xxh128 = 需 XXH3-128 实现 ✗ 见
  `golden/download_wire_r9.md`）；文件校验和 = **`MD5(内容)` 无 seed**（streaming `sum_update`
  只喂数据、seed 仅进块校验和 `get_checksum2`）——真机转录逐字节验证命中。
- **ndx 对齐（本可翻车的隐形坑）**：接收端请求的 ndx = **排序后下标**（`generator.c:2804` 遍历
  `cur_flist->sorted`）；实现 `sort_flist` 按 rsync `f_name_cmp` 真机序（同目录**文件先于子目录、
  各按名升序、遇目录深度优先下钻** ✗ 官方 `--list-only` 输出序实证）重排后再编码 ✗ 并用
  多文件递归下载 + 逐文件 SHA 比对证实映射正确（若错位 = 发错文件而 md5 自洽，RC 仍 0）。
- **实装**：`send_files` 循环（ndx/iflags/BASIS/XNAME/sum_head+块读写 → 回显 → `int32(len)+data`
  32KB 分块 → `int32(0)` → MD5）；`FlatEntry.fs_path`；`handle_conn` 第四参 `read_file` 闭包
  （bin = `workspace.read_file_bytes`）；`write_ndx`/`data_int`/`data_shortint`/`data_vstring`；
  `md-5 0.10`（本机 registry 已有 = 离线）。
- **门禁**：`cargo test -p vfiles-rsync` **13/13**（新增：f_name_cmp 排序黄金、整文件下载整链逐
  字节、MD5 RFC 向量）× workspace 全绿 × clippy/fmt × bin build。
- **r11 债**：真 delta（现全 literal = 正确无收益）· push 收端 · secrets 认证 · 大文件流式（现全量
  入内存）。
...
- **对答表提取**（资产 = `golden/wire_dialogue_r8.md`）：解析 strace 八进制转义按 fd3 分
  write/read = **收流/发流两序** → 状态机直接照推 ✗ 收（banner/GOLD/**args 双 NUL**/
  `1E`+30B checksum 串/`04帧`+`01`+`03`+`01`）发（OK 后 **`81 FE`** → **`#xxh128…none\n`
  (36B字面)**+**`46 19 14 67`** → **按树的 57B flist** → 尾三帧 `01/02/10` 配对 ✗ 20B 10 帧
  含可疑摘要值 = 原样发+CLI 报错循环修记档）✗ 顶部 8×832B ELF = fd 复用启动噪声剔除 ✓。
- **两大语义一并解** ✨：① **`0x19` preserve 逻辑全通** = `--list-only` 接收端 `!preserve_uid`
  → SAME_UID|GID **恒置**（首条 = TOP|UID|GID 完全吻合 ✗ 差分位方案定稿：非首 `0x18` +
  SAME_MODE/TIME 按前条差分 + 不用 SAME_NAME 恒发全名）② **mode = 原生 stat mode u32 LE
  恒等实证**（golden `0x41FD`/`0x101B4` 两独立命中 = stat 原值 = `to_wire_mode` 不需源 ✗
  本轮 clone 120s 超时复发（网络不稳）也已无关）。
- **r9 终清单锁定**（实装+探针 = goal 收官路径）：ListCtx（vfiles-app/domain path house
  式）→ 五步状态机 → flist 编码器（差分位+varlong 紧凑+mode LE+递归树）→ duplex 字节断言
  单测 → #61 三跑 → bin 传 ctx → **终极探针 = 真 `rsync --list-only rsync://…/files/` =
  RC0 列出 vfiles 文件**（CLI 报错循环修）→ delta（r10）。
- 纪律：#62/#63/#65 全程（本批纯侦察 = 无长跑 ✗ 条件 verify-first 再验）。

### 4.205x rsync 编码法定稿 + mtime 解出（round 7/256 ✨ git clone 权威源、帧逐字段闭环）

- **通道胜利（r6 双败后的正解）**：`git clone --depth 1` 官方 rsync 到本机 = **完全避开
  web_fetch 30s 超时墙 + 反爬壳**（flake 一次、4003 行 flist.c 本地权威随手 grep ✗ 立法 =
  大源文件取权威一律 git clone 不走 web_fetch ✗ 入 RSYNC_PLAN 状态）。
- **权威编码法全集入档**（`golden/frame_codec_r7.md`）：**XMIT 18 位表**（rsync.h:47-73）·
  **字段写序双向源**（send L700-950 / recv L1002-1300 ✗ xflags→l1/l2→name→length→mtime→
  mode→uid→gid）· **varlong(min) 紧凑形**（`int_byte_extra[64]` 表：00-7F→0/80-BF→1/…
  F8-FF→6 ✗ **ctrl 在首判 extra、值 LE 尾接、ub 整读 LE** ✗ varint 同构 · int=平台 x86 LE
  · shortint=BE 混用记档）· varlong30/varint30 ≥30 直通（锁 30 ✓）。
- **mtime 解出 ✅✅（r7 核心）**：`6a 51 86 b3` → ctrl=0x6a(extra=0) + ub=`[51,86,b3,6a]` LE
  = **0x6AB38651 = 1790150225 = epoch 完全命中** ✗ r6 的"中间两对交错" = **ctrl 首 + 值尾**
  的紧凑布局假象（值域全扫零命中是因为字节序与布局双错位）✗ **57B 逐字段闭环**：首条 `.`
  = xflags 0x19(TOP|SAME_UID|SAME_GID 语义吻) + l2/name + length varlong30(min3)=**4096**
  与 ls 一致 ✓。
- **r8 实装无阻**（编码器可逐字节复刻）：args/`\x81\xFE`/`\x1E`/`#选定` 明文层 + **flist 帧
  编码（SAME_* 差分 + to_wire_mode 源同步）** + 终结配对对答 + 树→条目流 → 终极探针 =
  真 `rsync --list-only rsync://…/files/` = **RC0 列 vfiles 文件** → delta（r9）· 块头
  语义（`35…`+前段 `81 FE 46 19 14 67` 连读）= 实装时定、不阻条目层。

### 4.204x rsync 帧三字段反推 + P04 真 CLI 完胜（round 6/256 ✨ name/mode/size 铁证解、mtime 差一层）

- **P04 三重兑现终验** ✨：修（r5 `@ERROR: Unknown module '<name>'` 单行）→ 断言同步 →
  **r6 真 CLI 复验输出与官方 r5 记录逐字同**（三行→两行 = 形差清零 ✓）+ P01 = RC0 收尾金
  标准保持 ✗ 门禁 rsync 3 测 0.00s、BUILD=0、#60 前置逐断言、20227 up。
- **57B 帧三字段反推解**（资产 = `golden/frame_layout_r6.md` =三锚值域法）：**name = 1B 长度
  +bytes+\0（`.`/`sub`/`a.txt` pre 字节 01/03/05 三独立证实）· mode = u32 LE（0o40775/0o100664
  双命中、与 ls 的 drwxrwxr-x/-rw-rw-r-- 值一致）· size = u16（4096/13 双命中）** ✓✓ 头部
  `35 00 00 07 A0 19` = mux/flist 头层（r7 与 mtime 一并源对）。
- **mtime 差一层（如实）**：epoch 1790150225 的 u32 BE/LE、u64、±1 天、float32/64 全扫 = **零
  命中** ✗ 帧 `6a 51 86 b3`(@11-14) 与 epoch 字节 `6a b3 86 51` = **中间两对交错**（疑 16 位
  半段重排/varint 变形）✗ **双网络源同批尽失**：GitHub raw flist.c web_fetch 30s 超时 +
  manpage.me 反爬壳（2059B/312 词无正文）✗✗ → **r7 权威通道 = GitHub blob 带行号页**（渲染含
  源码行 ✗ flist.c L1841(L1841-1843 NSEC_BUMP/protocol 31)/L3204 send/read_file_entry 段 =
  mtime 写读路径一击）。
- **r7 清单锁定**：mtime 权威解（blob 行号页）→ 五层实装（args 解析 / `\x1E`+checksum `#`
  回 / mux+flist 编码 / 终结帧对答 / 树→条目流 collect_keys 复用）→ **终极探针 = 真
  `rsync --list-only rsync://…/files/` = RC0 列出 vfiles 文件** → delta（r8）。
- 门禁纪律：#62 串行 / #63 硬超时 / #65 五钟（本批一切秒级 ✓）。

### 4.203x rsync wire 转录全知（round 5/256 ✨ strace 直挂一把抓双端全字节 = 实装钥匙全齐）

- **P04 官方形修兑现（r4 承诺）**：未知模块回 `@ERROR: Unknown module '<name>'` 单行即关
  （与官方逐字节同形 ✗ 断言同步 ✗ 三黄金 0.00s 绿 ✗ 真 CLI 复验 r6 开工一并）。
- **#66 strace 立法（本批唯一事故）**：`strace timeout N cmd` = timeout fork 的 cmd **不在
  trace 里** → 只抓 ld.so 读 ELF 空转（一次全空 ✗✗）→ 改**直挂二进制 + `-f` + `-s 999999`**
  =一把抓到**双端全字节 wire 转录**（118 行 read/write 分片 = 34KB 原始入库
  `golden/client_strace_raw.txt` sha=968dc6cd…）→ 入 PROBE_IDIOMS #66。
- **wire 五层时序解剖完成**（`golden/wire_anatomy_r5.md`） ✨：① 明文握手（banner 41B 版本+
  算法 → `GOLD` → `OK`）= **我们已同形** ② **命令段真形** = `--server\0--sender\0
  -de.LsfxCIvu\0--list-only\0.\0GOLD/\0\0`（真 flag 簇 + 源 `.` + 模块内路径 ✗ r4 python
  模拟缺 flag 簇 = 对照差记档）③ 协议确认 = 服 `\201\376` → 客 **`\x1E`(=30)** + checksum
  串 → 服 **`#`+算法串+`\n`**（= 算法选定响应前缀）+ 4B ④ **flist 57B 铁证帧**：name =
  **1B 长度+bytes 双锚**（`03 sub\0` / `05 a.txt\0` ✓ mtime/mode = r6 按 rsync 源 flist.c
  对齐）⑤ 终结帧族（双 fd mux 对话 = 收尾层 r6 精解）。
- **r6 实装清单已锁**（五层照转录 + 终极探针 = 真 `rsync --list-only rsync://…/files/` =
  **RC0 列出 vfiles 文件**）：args 解析（NUL 双尾 + flag 簇）/ `\x1E`+checksum `#` 回 /
  mux+flist 编码（1B len+name、mode drwxr-xr-x 形）/ 终结帧对答 / 树→条目流（collect_keys
  同族复用）。
- 纪律：#62 串行 / #63 硬超时 / #65 五钟（本批一切秒级 ✓ P04 三测 0.00s、strace 秒归）。

### 4.202x rsync 收尾层金标准达标（round 4/256 ✨ 官方 daemon 黄金对照法全胜）

- **对照法立法（本批核心方法论）** ✨：本机起**官方 `rsync --daemon`** 最小 conf →
  python 扮客户端**同款三步双向 dump**（收 banner → 发 31 → 空行/选模块）→ 与自家
  逐字节列差异 =**四层差一次全暴露**（banner 算法串 / 模块行 15 列+tab / EXIT 收尾 /
  时机）→ 按黄金逐字节修 → 真 CLI 复验 = **方法论完胜、零猜测空间**。
- **四层差 → 三修（lib.rs）**：① banner 补算法串 `@RSYNCD: 30.0 sha512 sha256 sha1 md5
  md4`（官方同款 ✗ 客户端按可用协商）② 模块行 `{name:<15}\t\n`（`files          \t` 与
  官方 `GOLD           \t` 逐字节同构 = 15 列填充 + tab 分隔）③ 收尾 =
  `@RSYNCD: EXIT\n` + 关连接——**百年疑案破：NUL 是多路复用帧的 channel 字节、不是
  列表终结**（r3 把它当 terminator = RC5/code5 真身）。
- **金标准达标** ✨：`P01: rsync --list-only rsync://…/` = **RC0 干净退出、输出
  `files          \t`**（P01/P04/字节对话三式全复验 + 三黄金单测同步新形 + full 26 套
  230 过 ✗ 全程秒级 = #65 践行 ✓ #62 串行 / #63 硬超时全程）。
- **P04 形差暴露（r5 首件一行修）**：官方 = `@ERROR: Unknown module 'nope'`（单行即关
  ✗ 我方 `@RSYNCD: ERROR Unknown module` 多一句 didn't-get = 形差 + 断言同步）。
- **Phase1 黄金资产入库**（`crates/vfiles-rsync/golden/` 两件 + sha256 ✓）：选模块层 =
  `@RSYNCD: OK\n`（**我方已同形 ✓**）· args 形已知 =
  `--server\0--sender\0--list-only\0-v\0.\0\0`（NUL 分隔双 NUL 尾）· 官方首响应 =
  **5 字节 `06 fc 5d 11 67`**（mux 首帧待解剖 = r5 两参照：rsync 源 mux 定义 + 更完整
  客户端模拟）。
- 门禁：rsync crate 0.00s ×3 过 / full 26 套 230 过 / CT=0 / BUILD=0（一切跑秒级 ✗ 无
  一次分钟级 = 用户令五钟阈值全程兑现）。

### 4.201x rsync Phase0 探底（round 3/256 ✨ 真 CLI 认出我们的模块 + 三大事故入档）

- **批A config**：`RsyncConfig{enabled默认false, port873, module"files"}` 三 env +
  serde default + `test_config_load` 三断言（19 过 ✗ 对称 S3 显式启用先例）。
- **批B crate `vfiles-rsync`**：`handle_conn` 纯协议件（AsyncRead/Write 泛型 = duplex
  黄金单测可打）+ 三黄金式（banner 先发/31 降级 ✓ 模块列表 `files\n\0` ✓ OK/ERROR 双路
  ✓ 版本解析 ✓）+ bin `build_and_spawn_rsync`（accept loop select 停机 watch + r205
  降级 + 关时日志明示启用法 ✗ Cargo 连字符键第三见补齐）。
- **批C 门禁**：完整 `cargo test` **26 套 230 过 0 错 0 败** + check --tests + build --bin
  全绿（串行单 shell 式 = #62 后首战 ✓ 硬 timeout 验证 = #63 ✓）。
- **批D 真 CLI 三式** ✨：`P01: rsync --list-only rsync://…/ = files`（**真 3.2.7 打印
  我们的模块 = 协商+列表全通** ✓）· `P04: @RSYNCD: ERROR Unknown module` ✓ · python 裸
  socket banner=`@RSYNCD: 30.0\n` + `files` 字节断言 ✓ · 服务日志「rsync daemon 已拉起
  addr=0.0.0.0:20227 module=files」✓ · **边界 = 客户端 code5 @main.c:1885 `didn't get
  server startup line`**（列表收尾期待差一层 = r4 首步：官方 rsync --daemon 抓黄金字节
  对照 → file_list → delta 三层）。
- **三大事故入档（#62/#63/#64 入 PROBE_IDIOMS + saga）** ✗✗✗：
  1. **误判锁等 20 分钟**：rsync test 真死锁 = **临时 BufReader 逐行新建吞行后字节**
     （client 两连写 → server 首 BufReader 读到首 `\n` 即弃 → 请求行的 `\n` 被吞 → 互等；
     测试 1 的 NUL 同构）✗ 我归因"target 锁"连杀 N 轮 = 自伤（SIGKILL 杀到 q3 只剩
     10/24 套的**外壳 exit0 假绿** ✗✗）→ **根治 = 单实例 BufReader 包 stream、写走
     `get_mut()`、跨类型读走 reader**（一发 0.00s 绿 ✓ 下次 `timeout 60` 先行 #63）。
  2. **`pgrep -f` 自匹配杀自己二犯**（模式串在自身 argv = #56 换形 ✗ 括号式/`-x` 才安全）。
  3. **cargo 跨工具并发互锁**（同消息多条 = 锁队列 + 排队中清场互杀 = 600s 反复上演 ✗
     正式 = 单 shell `&&` 串行、跨消息绝不并行）。
- goal 判定：S3 全链完成（r1-r2 ✓）✗ **rsync = Phase0 进取达成但未完整通**（差收尾层 +
  Phase1/2）→ **goal 留 active（r4 续）**。

### 4.200x S3 九式实证 + rsync 选型（round 2/256 · 装配+实证轮 ✨ 9/9 全绿）

- **批A config**：`S3Config{enabled默认false,port9000,access,secret}` 四 env（显式启用
  原则 + `S3Config::bind_address` + serde default 两 fn + `test_config_load` 加两断言 ✗
  #61 三跑护航）。
- **批B 装配**：`EnvAuth`（s3s::auth::{S3Auth,SecretKey} 权威式 = doc 例带路 ✗ SimpleAuth
  也 pub 可换记档）+ `build_and_spawn_s3`（S3ServiceBuilder→set_auth→HandleError→
  fallback_service→with_graceful_shutdown ✓ r205 式 bind 失败 error 降级不拖垮 ✗ 无键
  uuid 随机 + warn 打印 access、secret 不落日志）+ **依赖序矛盾**（S3 调用需 upload 未
  move（AppState 前）+ watch rx 已定义（原 AppState 后）✗ **watch 定义整体前移解**）+
  关时日志明示启用法 ✗ Cargo 键名 =包名连字符（`vfiles-s3` key / `vfiles_s3` 引用 =
  help 直给 ✗ r1 manifest 同族）。
- **批C 九式 9/9 PASS** ✨✨（`crates/vfiles-s3/scripts/sigv4_probe.py` 入仓可复演）：
  ListBuckets/Objects 200 · **PUT 版本 v0→1 · 同名 PUT v1→2（产品核心跨协议实证 ✓✓）**
  · GET+ETag `"bfc3…"` · HEAD CL · DELETE 幂等 204/204 · NoSuchBucket 404 XML · 错
  secret 403 · db 联动断言（entry_versions 直查 ✗ #60 前置逐断言：build→init→user→
  import→serve→port-wait→db 卫生）。
- **签名真容剥七层 saga** ✗：①服务日志三行权威 ERROR `SignatureDoesNotMatch` =验签在跑
  ✗ 真因 = heredoc **`"\n".join` 字面双反斜杠**（canonical 拼接全废）→ `chr(92)` 零转义
  修（**修探针自身也要防转义地狱**）② 404 簇 = **path-style bucket 段缺**（式8 的
  `/wrongbucket` 404 反证 bucket 段生效 =探测面互证 ✓）③ 式5 etag=None = **r1 设计写了
  方案 impl 漏赋字段**（设计≠实现检查缺位）→ 补 find_by_path 双查 + `e_tag`（真名 ≠
  etag，编译器 similar help 带路）④ `Option<Entry>` 解包 + **`ETag` 是枚举**
  （Strong/Weak ✗ 引号 = `to_http_header` 自附 = 我存裸 hex 防双引 = 引号单所有者 ✓）⑤
  **式9 假阳警示**：1-8 全 403 时式9 也 PASS = 探针前置不自足（#60 形态再证 ✗ 断言依赖
  上游成真）。
- **批D 两计划落档**：`docs/S3_PLAN.md`（形态表/九式账/启用接入法/八项扩展债：per-user
  凭证·续页·delimiter·流式 put·last_modified·multipart·health·region）+ `docs/RSYNC_PLAN.md`
  （**选型定案：自研 rsync-30 + daemon 形** ✗ 无成熟 Rust crate（HN 纯 Rust 在途=参照）✗
  借道 C rsync 不可行（存储在 sqlite 非裸 fs）✗ r3 首版只读拉 + 真 rsync CLI 一发实证
  探针预告）。
- **门禁四跑全绿**（check/--tests/test/build --bin ✗ 24 suites ✗ **#61 纪律 r2 首战全程
  护航** = test mod cfg 两补后工作区真双净持续保持）。

### 4.198x S3 兼容 API 开局（round 1/256 · 新目标 spike 轮 ✨ impl 编译绿）

- **目标**：近期路线双协议 = S3 兼容 API（首件）+ rsync（选型 r2 续）。**r1 交付 =
  侦察 + s3s 接入 + VfilesS3 impl 编译绿**（装配/实证/文档 = r2 批3-5续 ✗ rsync 选型注
  同 r2）。
- **侦察定案** ✨：s3s 0.16.1（本地实读 ✗ `S3ServiceBuilder→tower Service→axum
  fallback_service` 集成式权威例 ✓ trait 114 方法**全默认 NotImplemented** = 只需
  override 6 个 ✓ SimpleAuth/set_auth 认证面 ✓ `ContentType=String` 透传型 ✓
  `S3Response::new` 官方构造（手填缺 extensions 被编译器抓 ✗）✓ dto 全在 generated.rs
  大文件 =**写-编译-按 available 字段修**的教学循环全程复用 ✓）。
- **设计定案**：独立端口（path-style 与前端根语义冲突 ✗ MinIO 9000 直觉 ✓）+
  单虚拟桶 `default` 严格语义（非 default → NoSuchBucket ✓）+ **env 静态单对 SigV4**
  （未设随机生成 + warn ✗ per-user 凭证 = 扩展债）+ key = 默认 ns 相对路径（树展平
  collect_keys 递归 ✗ ETag 设计 = version-id hex 引号（与 WebDAV r14 跨协议同式 ✗
  last_modified=None 记 P2））+ put 聚合（流式直连 P2 债 = WebDAV 同级）。
- **impl 六方法**（list_buckets/list_objects_v2/get/head/put/delete ✗ S3 DELETE 幂等
  204 ✓ content_type 直透 init_upload(as_deref) ✓）——**7 错→修→绿**循环记录：
  S3Error 名（`s3s::Error` 不存在✗ error::mod真名）/ head 参数型 / ?转换 DomainErr
  无 From（改 s3_error! 直映）/ i32 / 枚举型（storage_class 省赋值=免型）/ if-else 型
  / as_deref 编译器式 ✓ **错由编译器全程带路 = 侦察→落笔→绿零猜空击**。
- **意外收益（警告清零）** ✨：cargo fix --workspace 5 修 + 手写 Debug 非省内容式 +
  r12 死变量（destination_is_directory 连锁 destination_entry）+ webdav 3 遗留 import
  —— **WS_CLEAN 零错零警告**（既有债顺手全清 ✓ lint 面与 Rust 面双净复位）。
- **Manifest saga**：`workspace= true` 无根键（根无 vfiles-* 键 ✗ webdav 式 =path 直引
  四依赖改 ✓）+ s3s 直版 0.16（workspace 无键 ✗ 删无效行）+ 编译器 `Caused by` 三层
  追因（parse→inherit→not found = **错误链逐层读到底**纪律 ✓）。
- 全门禁：`cargo check`（workspace）**零错零警告** ✅ crate check 绿 ✅。

- **【轮尾补记 = 本轮最大教训 #61】** ✗✗✗：`cargo fix --lib` 按 **lib 视角**判 unused
  → **把 cfg(test) mod 里正在使用的三处 use 删了**（child_prefix/entry_href/if_token）✗
  而 `cargo check` **同样不编 test 面** = WS_CLEAN/"零警告" 与 cargo test **101 真炸**
  并存 = **test 面假绿** ✗✗✗ 按编译器 help 复原两 use → `cargo check --tests` = 0 +
  全 workspace `cargo test` = 24 套 227 过 + webdav 25 过 = **双面真绿**（复原后 amend
  同 commit ✓）→ **新纪律入册（#61，与 #52 build 并列）：凡动 Rust 源必
  `cargo check --tests` + `cargo test` 双跑**——"check 绿"永远含 tests 面才算数 ✗
  **计数基修正**：`git rev-list origin/master..HEAD` = **248 实算**（估算退役 ✗ 以后
  一律实算）。
...
- 原「disabled 每命中实时查」和「改密码 30s 窗」结论不成立：缓存命中会直接返回旧 `User`，并未读取数据库。WebDAV Litmus 104 项通过后复核共享认证路径时发现，停用和密码重置在 30 秒 TTL 内仍可接受旧 Basic 凭据。
- 现在命中缓存仍按用户名或邮箱读取当前账号记录，并核对用户 ID、启用状态和密码哈希；改名、改密、停用或删除在下一次请求即时生效，Argon2 热路径仍被跳过。`vfiles-app` 增加缓存后改密与停用的回归测试。
