...
- 自写 SigV4 HTTP 探针在隔离 `/s3` 实例通过 **27/27**；安装到独立临时环境的 boto3 真实客户端探针通过 **58/58**，覆盖根路径与 `/s3` 前缀的 SigV4 path-style endpoint、对象 API、分页、删除标记前置过滤、multipart、Range、条件请求及严格 region 下的预签名 URL。
...
- **实装**（网关只有唯一虚拟桶 ✗ 错误码严格对齐 AWS）：
  | 操作 | 情形 → 响应 |
  | --- | --- |
  | `CreateBucket` | 服务桶区域为 `us-east-1`，重复创建 `default` → **200** 且返回 `Location: /default`；其他桶名 → **400 `InvalidBucketName`**（本网关单桶模型） |
...
- **原验收记录已更正**：AWS `us-east-1` 对自有桶重复创建返回 200；之前误按其他区域的 `BucketAlreadyOwnedByYou` 409 作为预期。boto3 probe 已改为验证 idempotent success。
- **真机验收（真 SDK，两台配置合计 9 项）**：`create(default)`=200 ✓ · `create(其他)`=InvalidBucketName ✓ ·
