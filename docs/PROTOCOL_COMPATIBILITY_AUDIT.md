...
| QA-002 | WebDAV | cadaver 0.24 passed authenticated MKCOL, PUT, COPY, MOVE, PROPFIND listing, GET byte comparison, DELETE, and collection removal; the sequence is now reproducible with `crates/vfiles-webdav/scripts/cadaver_probe.sh` in CI. rclone 1.60.1-DEV also passed MKCOL/PUT/list/GET. | P1 | Automated real-client coverage; full litmus suite remains open |
