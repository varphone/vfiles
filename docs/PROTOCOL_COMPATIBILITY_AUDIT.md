...
| QA-003 | RSYNC | System rsync 3.2.7 protocol 31 is available, but this cycle did not run it against the daemon and the repository has no automated real-client test. | P1 | Open |
...
- Manual client checks in this environment are evidence for the operations
  ...
  scenarios and add real rsync daemon client coverage as follow-up work.
