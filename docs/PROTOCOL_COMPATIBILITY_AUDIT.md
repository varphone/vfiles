...
| QA-003 | RSYNC | System rsync 3.2.7 protocol 31 passes the isolated daemon probe: module discovery, recursive push/pull with empty directories and a 2 MiB file, same-size checksum update sending less literal data than the full file, `--delete --exclude` protection, and `--delete-excluded`; pulled trees are compared byte-for-byte. | P1 | Real-client probe runs in CI; broader rsync feature coverage remains open |
