...
| QA-002 | WebDAV | cadaver 0.24 passed authenticated MKCOL, PUT, COPY, MOVE, PROPFIND listing, GET byte comparison, DELETE, and collection removal. System litmus passed all 104 basic/copymove/props/locks/http checks, including shared and depth-infinity locks; its two warnings are informational: the HTTP URI parser removes fragments before the DAV handler (so they cannot select a distinct resource there), and litmus expects 423 for a false `If` condition although RFC 4918 §10.4 requires 412. | P1 | Full installed litmus suite verified; cadaver probe is automated in CI; RFC status checked |
