...
- PUT 在读取请求体前校验 If/锁条件，并在 SQLite 写入事务中复核活动锁令牌集合；上传期间新增、移除或过期的覆盖锁都会使旧快照失效，防止长上传跨过锁状态变化提交。
...
- 认证成功缓存命中时仍重新读取当前账号记录，并核对账号 ID、启用状态和密码哈希；管理员停用、改密、改名或删除账号后，下一次请求立即拒绝旧凭据，同时避免重复执行 Argon2。
