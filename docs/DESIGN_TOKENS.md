...
**双线分工注**（r190 ✓）：JS 布局线（`matchMedia max-width:1023`）= **布局级切换**
（列表/卡片形态）与表 1024 桌面档**对齐** ✓；768/480 = **组件级微调**（表单/侧栏折叠）。
布局判定用 1024 线、组件适配用 768/480 档，勿混用。
...
| 层 | 值 | 用途 |
| --- | --- | --- |
| 局部 | 0–6 | 卡内/表内/sticky 件（局部上下文 ✓ 有意微层 ✗ 0 = 表扩（r189 实测）） |
| 浮层 | **30** | 面板内浮层（搜索面板等；r94 归一 25→30 ✓） |
| 菜单浮层 | **60** | chip / HUD / 右键菜单 / 面包屑（×6 一致档） |
| 模态 | **100 / 1000** | 业务对话框 100 / **覆盖确认框 1000**（r189 表修订 ✓ 现实双层语义：全局确认盖业务对话框 ✗ 原表 1000 单层记 ✗） |
| 内容覆盖 | **3000** | 拖拽覆盖层（全屏盖 ✓） |
