...
vi.mock('../src/services/auth.service', async () => {
  ...
  return {
    authService: {
      ...actual.authService,
      listUsers: vi.fn(async () => ({ success: true, data: { users: mockUsers } })),
...
describe('AdminUsers.vue', () => {
  it('loads and displays users, saves email and revokes sessions', async () => {
    const { findByText, getByPlaceholderText, getAllByText } = renderWithProviders(AdminUsers);

    await findByText('用户管理');
    await findByText('alice');

    const emailInput = getByPlaceholderText('user@example.com') as HTMLInputElement;
    ...
    expect((authService.setUserEmail as any).mock.calls.length).toBeGreaterThanOrEqual(1);
    const [calledId, calledEmail] = (authService.setUserEmail as any).mock.calls[0];
    ...
    expect((authService.revokeUserSessions as any).mock.calls.length).toBeGreaterThanOrEqual(1);
  });
});
