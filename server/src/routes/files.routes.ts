...
export function createFilesRoutes(gitService: GitService) {
  ...
  function isAllowedFileTypeByNameAndMime(filename: string, mime?: string): boolean {
    const allowed = config.allowedFileTypes;
    if (!allowed || allowed.length === 0) return true;
  ...
  async function listReceivedChunks(uploadId: string, totalChunks: number): Promise<number[]> {
  ...
  async function saveSession(session: UploadSession): Promise<void> {
    ...
    await fs.writeFile(getUploadSessionPath(session.uploadId), JSON.stringify(session), 'utf-8');
  }

  async function cleanupExpiredSessions(): Promise<void> {
    try {
      await fs.mkdir(config.uploadTempDir, { recursive: true });
      const entries = await fs.readdir(config.uploadTempDir, { withFileTypes: true });
      const now = Date.now();
  ...
  app.post('/upload/init', async (c) => {
    ...
    try {
      ...
      const filenameResult = validateRequiredString(body?.filename, 'filename', { maxLength: 255 });
      if (!filenameResult.ok) {
        return c.json({ success: false, error: filenameResult.message }, filenameResult.status);
      }
      ...
      if (size > config.maxFileSize) {
        return c.json(
          { success: false, error: `文件过大，最大允许 ${(config.maxFileSize / (1024 * 1024)).toFixed(0)}MB` },
          413
        );
      }

      const lastModified = body?.lastModified != null ? Number(body.lastModified) : undefined;
      ...
      const filePath = requestedPath ? `${requestedPath}/${filename}` : filename;
      ...
      const chunkSize = Math.min(Math.max(1, config.uploadChunkSize), config.uploadMaxChunkSize);
      ...
      if (existing) {
        // 若参数不一致，视为新会话并清理旧分块
        if (existing.filePath !== filePath || existing.size !== size || existing.chunkSize !== chunkSize) {
      ...
      return c.json(
        { success: false, error: err instanceof Error ? err.message : '初始化失败' },
        500
      );
    }
  });
  ...
  app.post('/upload/chunk', async (c) => {
    try {
      ...
      return c.json(
        { success: false, error: err instanceof Error ? err.message : '上传分块失败' },
        500
      );
    }
  });
  ...
  app.post('/upload/complete', async (c) => {
    try {
      ...
      if (!isAllowedPathByPrefixes(session.filePath, config.allowedPathPrefixes)) {
        return c.json({ success: false, error: '不允许写入该路径' }, 403);
      }
      ...
      return c.json(
        { success: false, error: err instanceof Error ? err.message : '合并失败' },
        500
      );
    }
  });
