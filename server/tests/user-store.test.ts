...
import { UserStore, hashPassword, verifyPassword } from '../src/services/user-store.js';
...
describe('UserStore', () => {
  ...
  it('create and fetch user by username/email', async () => {
    const u = await store.createUser({ username: 'alice', password: 'pass1234', email: 'alice@example.com' });
  ...
  it('resetPassword changes hash and bumps sessionVersion', async () => {
    const u = await store.createUser({ username: 'userd', password: 'oldpass' });
