...
impl Http1Transaction for Server {
    ...
    fn parse(buf: &mut BytesMut, ctx: ParseContext<'_>) -> ParseResult<RequestLine> {
        ...
        let uri = {
            ...
            // Request targets are origin-form, absolute-form, authority-form, or `*`;
            // none includes a URI fragment. Reject it before `http::Uri` normalizes it
            // away, otherwise a destructive route could act on a different target.
            if uri_bytes.as_ref().contains(&b'#') {
...
mod tests {
    ...
    fn test_decoder_request() {
        ...
        // A raw fragment is not part of an HTTP request target. Reject it before URI
        // normalization can turn DELETE /resource#other into DELETE /resource.
        parse_err(
            "\
             DELETE /resource#other HTTP/1.1\r\n\
             host: example.test\r\n\
             \r\n\
             ",
