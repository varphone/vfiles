...
impl S3ServiceBuilder {
    ...
    /// Sets an optional HTTP path prefix for S3 operations.
    ///
    /// The prefix is removed only for operation path parsing. SigV4 verification
    /// continues to use the original request URI, as required by AWS signing.
    /// Requests outside the prefix continue to use the root S3 path layout.
    /// The prefix must start with `/` and must not end with `/` (except `/`).
    pub fn set_path_prefix(&mut self, path_prefix: &'static str) {
...
fn make_request(req: HttpRequest, path_prefix: Option<&'static str>) -> Request {
    let mut req = Request::from(req);
    if let Some(path_prefix) = path_prefix {
        req.extensions.insert(crate::http::PathPrefix(path_prefix));
    }
    req
}
...
impl S3Service {
    ...
    pub async fn call(&self, req: HttpRequest) -> Result<HttpResponse, HttpError> {
        ...
        let mut req = make_request(req, self.inner.path_prefix);
...
mod tests {
    ...
    fn future_size() {
        ...
        assert!(output_size(&S3Service::call) <= 3328);
        assert!(output_size(&S3Service::call_owned) <= 3632);
    }
