    ...
    */
    ...
    fn convert_poll_message(
        ...
        if let Some(path) = pseudo.path {
            ...
            // `:path` carries the HTTP request target, not a generic URI. Reject raw
            // fragments before `PathAndQuery` truncates them during normalization.
            if path.as_ref().contains(&b'#') {
                malformed!("malformed headers: fragment in :path");
            }
...
mod request_target_tests {
    use super::Peer as ServerPeer;
    use crate::frame::{Pseudo, StreamId};
    use crate::hpack::BytesStr;
    use crate::proto::Peer as PeerTrait;
    use http::{HeaderMap, Method};

    #[test]
    fn request_parser_rejects_raw_fragment_and_accepts_encoded_hash() {
        let request = |path| {
            <ServerPeer as PeerTrait>::convert_poll_message(
                Pseudo {
                    method: Some(Method::GET),
                    scheme: Some(BytesStr::from_static("https")),
                    authority: Some(BytesStr::from_static("example.test")),
                    path: Some(BytesStr::from_static(path)),
                    ..Default::default()
                },
                HeaderMap::new(),
                StreamId::from(1),
        ...
        assert!(request("/dav/item#other").is_err());
        assert!(request("/dav/item%23other").is_ok());
        assert!(request("/dav/item?query=value").is_ok());
    }
}
