...
### v1.1.2 - 邮件系统（忘记密码 / 验证码登录）（计划）

- [ ] 邮件发送基础设施：SMTP 配置 + 发送封装 + 失败重试/降级提示
- [ ] 忘记密码（后端）：申请重置（发邮件）/ 校验 token / 设置新密码（使旧会话失效）
- [ ] 邮件验证码登录（后端）：申请验证码（发邮件）/ 校验验证码并登录
- [ ] 安全与风控：对“发邮件/验证码校验/重置密码”做限流（按 IP + 账号）
- [ ] 数据与清理：验证码/重置 token 的有效期、一次性使用、存储与定期清理
- [ ] 前端页面：忘记密码入口、重置密码页、验证码登录切换与状态提示
- [ ] 文档：新增环境变量与部署说明（SMTP、有效期、限流参数）
